...
from okean import roms, netcdf, calc


class RomsGlider:
    def __init__(self, f, x, y, t):
        self.fname = f
        self.x = x
        self.y = y
        self.t = t
    ...
    def locate(self, varname):
        ...
        for c in range(ntimes):
            ...
            # Find time indices.
            if (self.t[c] >= self.roms.time[0] and self.t[c] <=
               self.roms.time[-1]):
            ...
            # isIn = False
            for I in [i-1, i]:
                if I+1 >= lon.shape[0] or I < 0:
                    continue
                for J in [j-1, j]:
                    if J+1 >= lon.shape[1] or J < 0:
                        continue
        ...
        inds = [(Iind[k], Jind[k], Tind[k]) for k in range(len(Iind))]
    ...
    def extract(self, varname, method='fast', nfast=1):
        ...
        if tag in self.uinds.keys():
            ...
            inds0, uinds = self.locate(varname)
        ...
        if not self.roms.hast(varname):
            ...
            no_time = False
        # Faster but will download more data than needed!
        if method == 'fast':
            ...
            if not calc.isiterable(nfast) and nfast == 1:
                ...
                for k in range(len(tmp)-1):
                    ...
                    vtmp = netcdf.use(self.nc, varname,
                                      xiSEARCH=II,
                                      etaSEARCH=JJ,
                                      ocean_time=tt)
                    if v is not False:
                        if vtmp.ndim < v.ndim:
                            # If len of last tt is 1!
                            vtmp = vtmp[np.newaxis, :]
                        v = np.vstack((v, vtmp))
                    else:
                        v = vtmp
            ...
            for i in range(I.size):
                ...
                if T[i] >= 0:
                    tind = T[i] - T[T >= 0].min()
                # Negative means data outside model time.
                else:
                    tind = T[i]
        ...
        for i in range(len(self.t)):
            ...
            tind = T0[i]
            if tind < 0:
                # Data outside model time.
                continue

            # Rotate cell before interpolation.
            xp = np.asarray([lon[eta, xi], lon[eta, xi+1],
                             lon[eta+1, xi+1], lon[eta+1, xi], self.x[i]])
            yp = np.asarray([lat[eta, xi], lat[eta, xi+1],
                             lat[eta+1, xi+1], lat[eta+1, xi], self.y[i]])
            ...
            u0 = ((V[j0]*a*b + V[j1]*A*b + V[j2]*A*B + V[j3]*a*B) /
                  (a*b + A*b + A*B + a*B))
            ...
            if not no_time:
                ...
                u1 = ((V[k0]*a*b + V[k1]*A*b + V[k2]*A*B + V[k3]*a*B) /
                      (a*b + A*b + A*B+a*B))
    ...
    def depth(self, varname='r'):
        h = self.extract('h')
        zeta = self.extract('zeta')
